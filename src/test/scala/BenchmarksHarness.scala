...
import sygus.{Main, PySynthesisTask, PythonPBETask}
import ast.ASTNode
...
//    val dirname = "src/test/resources/bester_benchmarks"
    ...
    for ((file,idx) <- dirFiles.zipWithIndex) yield {
      ...
      val (programs, time) = Main.pySynthesize(file.getAbsolutePath)
      val t1 = System.currentTimeMillis()
      val origFilename = filenameToGoldStandard(file.getName)
      val task: PySynthesisTask = PythonPBETask.fromString(fromFile(file.getAbsolutePath()).mkString, true)
      ...
      file.getName + resultPrinter(programs, goldStandard, t1 - t0)
    }
  }
  ...
  val toohard = false || cmd.hasOption('h')
