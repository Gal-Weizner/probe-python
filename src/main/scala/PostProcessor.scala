...
object PostProcessor
{
  def clean(node: ASTNode): ASTNode = if (!node.usesVariables && node.values.toSet.size == 1) //second check is a tad redundant but just to be safe
    Types.typeof(node.values(0)) match {
      case Types.PyString => new PyStringLiteral(node.values(0).asInstanceOf[String],node.values.length, node.predicates)
      case Types.PyBool => new PyBoolLiteral(node.values(0).asInstanceOf[Boolean],node.values.length, node.predicates)
      case Types.PyInt => new PyIntLiteral(node.values(0).asInstanceOf[Int],node.values.length, node.predicates)
      case _ => node
    }
  else node match {
    case add: PyIntAddition =>
      ...
      (lhs, rhs) match {
        case (a: PyIntLiteral, b: PyIntLiteral) => new PyIntLiteral(a.value + b.value, a.values.length, add.predicates)
        case _ => new PyIntAddition(lhs, rhs, add.predicates)
      }
    case sub: PyIntSubtraction =>
      ...
      (lhs, rhs) match {
        case (a: PyIntLiteral, b: PyIntLiteral) => new PyIntLiteral(a.value - b.value, a.values.length, sub.predicates)
        case _ => new PyIntSubtraction(lhs, rhs, sub.predicates)
      }
    case sub: PyIntDivision =>
      ...
      (lhs, rhs) match {
        case (a: PyIntLiteral, b: PyIntLiteral) => new PyIntLiteral(a.value / b.value, a.values.length, sub.predicates)
        case _ => new PyIntDivision(lhs, rhs, sub.predicates)
      }
    case concat: PyStringConcat =>
      ...
      (lhs, rhs) match {
        case (a: PyStringLiteral, b: PyStringLiteral) => new PyStringLiteral(a.value + b.value, a.values.length, concat.predicates)
        case _ => new PyStringConcat(lhs, rhs, concat.predicates)
      }
